    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <Card className="bg-gradient-to-br from-ink-900 to-ink-800 text-white border-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold">Administracja i bezpieczeństwo</h2>
            <p className="text-sm text-white/60">Role użytkowników, kontrola dostępu i historia zmian</p>
          </div>
        </div>
      </Card>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="p-4">
          <div className="flex items-center gap-2 text-ink-500"><Users size={16} /><span className="text-xs font-medium uppercase tracking-wide">Role</span></div>
          <p className="text-2xl font-bold text-ink-900 mt-1">{roles.length}</p>
        </Card>
        <Card className="p-4">
          <div className="flex items-center gap-2 text-ink-500"><Lock size={16} /><span className="text-xs font-medium uppercase tracking-wide">Reguł dostępu</span></div>
          <p className="text-2xl font-bold text-ink-900 mt-1">{accessControls.length}</p>
        </Card>
        <Card className="p-4">
          <div className="flex items-center gap-2 text-ink-500"><History size={16} /><span className="text-xs font-medium uppercase tracking-wide">Zdarzeń w historii</span></div>
      ...
      {/* Roles overview */}
      <Card>
        <CardHeader title="Role użytkowników" subtitle="Profile dostępu w systemie" />
      ...
      <Card className="overflow-hidden p-0">
        <div className="p-5 border-b border-ink-100">
          <h3 className="text-base font-semibold text-ink-900">Macierz kontroli dostępu</h3>
          <p className="text-sm text-ink-500 mt-0.5">Uprawnienia odczyt/zapis/usuwanie dla każdej roli i zasobu</p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-ink-50 text-xs text-ink-500 uppercase tracking-wide">
              <tr>
                <th className="text-left font-medium px-4 py-3 sticky left-0 bg-ink-50">Zasób</th>
                {roles.map((r) => (
            ...
            <tbody className="divide-y divide-ink-100">
              {RESOURCES.map((res) => (
                <tr key={res} className="hover:bg-ink-50/50">
                  <td className="px-4 py-3 font-medium text-ink-800 sticky left-0 bg-white">{RESOURCE_LABELS[res]}</td>
                  {roles.map((r) => {
                    const a = getAccess(r.id, res);
                    return (
                      <td key={r.id} className="px-3 py-3">
                        <div className="flex items-center justify-center gap-1.5">
                          <PermIcon active={!!a?.can_read} icon={<Eye size={13} />} title="Odczyt" />
                          <PermIcon active={!!a?.can_write} icon={<Pencil size={13} />} title="Zapis" />
                          <PermIcon active={!!a?.can_delete} icon={<Trash2 size={13} />} title="Usuwanie" />
                        </div>
                      </td>
                    );
                  })}
        ...
        <div className="p-3 border-t border-ink-100 flex items-center gap-4 text-xs text-ink-500">
          <span className="flex items-center gap-1"><Check size={12} className="text-emerald-600" /> Dozwolone</span>
          <span className="flex items-center gap-1"><X size={12} className="text-ink-300" /> Zabronione</span>
        </div>
      </Card>
      ...
      <Card className="overflow-hidden p-0">
        <div className="p-5 border-b border-ink-100">
          <h3 className="text-base font-semibold text-ink-900 flex items-center gap-2"><History size={18} /> Historia zmian</h3>
          <p className="text-sm text-ink-500 mt-0.5">Audytorium ostatnich modyfikacji w systemie</p>
        </div>
        {changeHistory.length === 0 ? (
          <p className="p-5 text-sm text-ink-400">Brak zarejestrowanych zmian.</p>
        ) : (
